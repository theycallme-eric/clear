...
Companions: `DAG.md` (the graph), `requirements/REQUIREMENTS.md` (the frozen baseline),
`github/README.md` (how the issues got there).
...
next wave, thirteen. `DAG.md` has the full shape.
...
2. **Then highest fan-out.** `DAG.md` § *Highest leverage* ranks issues by how many they
   unblock. Taking `DS-01` (10 dependents) before `ENV-05` (0) opens the graph faster.
3. **Then whatever is on the critical path.** `DAG.md` names it. A delay anywhere else is
...
4. **`specs/design/ATOMIC.md`** — for any UI issue, always. It lists every token,
...
5. **`specs/IA.md`** — for a screen issue: route, guard, states, composition, atmosphere.

Do **not** read `requirements/REQUIREMENTS.md` to work an issue. Once issues exist it is a
...
**Do not:** widen the scope to cover the gap. Do not edit `requirements/REQUIREMENTS.md`.
...
python3 scripts/gen-dag.py        # regenerate DAG.md from the requirements
...
**Before issues exist,** `requirements/REQUIREMENTS.md` is the source of truth and the
