...
|---|---|---|---|
| Backend disposition approval (REQ-008) | any live-mutating DATA issue | Complete `docs/backend/live-inventory.md` §5 capture and the snapshot; keep all work author/dry-run | Approve `docs/backend/dispositions.md` (issue #80) so live mutation may begin |
...
| Reused Supabase project | DATA-01a | Write and locally verify migrations; author/dry-run only until the gate clears | Authorize CLI access to the reused live project (`qxckevxniacktaqecypl`) and approve `docs/backend/dispositions.md`; personal/test rows are never carried forward |
| Live catalog export | DATA-02 | Reconstruct and audit the 140-row catalog committed in the archived repo | Grant read access to the reused project so the expected 173 live rows can be exported and reconciled |
