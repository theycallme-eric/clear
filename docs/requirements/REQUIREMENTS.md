...
| 0.4 | **Two verified defects (D5, D6) and three outside review rounds.** Structured prescriptions, first-class blocks, three-state model with temporal lineage, duration plausibility guardrail, user-authored constraints, staged taxonomy migration. Detail lives in `docs/specs/DATA_MODEL.md` and `docs/specs/generation/GENERATION_CONTRACT.md`. `DATA-04`, `GEN-07`, and `META-01` were proposed during review and withdrawn before issue — hence the gap between `DATA-03` and `DATA-05`. |
| 0.5 | **`clear-design-system@0.5.0` landed and §9 was rewritten against it.** The export ships a component library, not tokens — DS-03 deleted, DS-01/02/05/06 rescoped from *build* to *integrate*, DS-07 shrunk, **DS-08** (adherence gate in CI) added. GEN-05 and SET-01 rescoped. **D7 registered** (set logs written live with no failure handling) and closed by **EXE-07**. **All eight backlog stubs resolved** — five cut, two promoted (EXE-06, EXE-07), one folded into DATA-02; reasoning in `docs/requirements/DEFERRED.md`. Substrate contract: `docs/specs/design/ATOMIC.md`. |
...
**0.2:** Progressive-overload spec landed (`docs/specs/OVR-01_progressive-overload.md`) → OVR stub promoted to four full requirements (OVR-01…04); DATA-01 and EXE-04 patched so the schema and logging are progression-ready from day one.
...
An oversized node causes **false serialization** — a dependent waiting on parts of a parent it does not actually need. The graph must not lie about the plan.

**Applied in v0.6.** Six requirements were split: `DATA-01`, `GEN-02`, `SES-01`, `EXE-04`, `OVR-01`, `DS-04`. Each split had to earn itself by naming the dependent it unblocks — DATA-02 waiting on execution tables it never reads, GEN-06 waiting on a model call it has nothing to do with, HOME-01 waiting on a whole session lifecycle when it needs one streak query. Splitting for tidiness alone is how a backlog grows without the plan improving.

The children are **peers, not sub-issues.** A parent tracking issue with no acceptance criteria of its own never closes and would surface in `is:open -is:blocked` as work that is not work. The sub-requirement IDs (`DATA-01a`…) carry the traceability that hierarchy would have; shared context and any criteria applying to all of them are reproduced in each child's body by `scripts/gen-issues.py`.
...
> **Independence rule.** The rebuild is self-contained. Every spec, asset, prompt, and token file it needs lives in this workspace under `docs/specs/` or `docs/design/`. **No requirement sends anyone to the archived `clear-app` repository.** If something is missing here, that is a bug in this document — say so rather than reaching backward. Duplication is the intended cost.
...
Terms this document uses precisely. Where a definition has a mechanism, it lives in `docs/specs/DATA_MODEL.md`.
...
|---|---|---|
| `docs/specs/DATA_MODEL.md` | Schema: blocks, three-state lineage, structured prescriptions, constraints, taxonomy migration | DATA-01…05, SES-01, REV-02/03, EXE-02…05, FAV-01, HOME-02/03 |
| `docs/specs/generation/GENERATION_CONTRACT.md` | Pipeline, candidate retrieval, output shape v4.1, validation split, duration algorithm | GEN-01…06, CORE-03, REV-02 |
| `docs/specs/IA.md` | 14 screen contracts, component vocabulary, navigation | every UI requirement |
| `docs/specs/OVR-01_progressive-overload.md` | Load anchors, RPE rules, deload triggers | OVR-01…04 |
| `docs/specs/structures/*.md` | EMOM, ladder/For Time, AMRAP, superset/circuit clarity | EXE-02…04 |
| `docs/specs/design/*.md` | Visual language, chamfer components, UI component spec | DS-01…07 |
| `docs/specs/screens/*.md` | Onboarding wireframe, loading screens | ONB-01, GEN-05 |
| `docs/specs/favorites-v2.md` | Personal bests, completion history, comparison | FAV-01/02 |
| `docs/specs/generation/exercise-swap.md` | Swap scope, history, undo | REV-02/03 |
| `docs/requirements/CHANGE_SET_v0.4.md` | The reasoning behind v0.4 — invariants, what was withdrawn and why | — |
...
|---|---|---|
| Claude Design (Pencil) export zip | DS-01 token source; DS-03…06 visual reference. **Gates DS-01, DS-02, DS-06** | Fall back to `docs/design/tokens/_fallback-from-old-repo/` + `docs/specs/design/visual-language-rules.md` — but see the gate note below |
| `docs/specs/` in this workspace | Every `port`/`rebuild` carry-over — all cited specs live here | Missing spec = defect in this doc |
...
| Coworker's DAG repo link | Format reference for generated docs/DAG.md | Non-blocking |
...
### ENV-07 — E2E harness + test data lifecycle
**Layer:** infra · **Milestone:** M0 · **Carry-over:** new
...
### DATA-01 — Baseline schema *(split: DATA-01a…d)*

The full baseline, authored against `docs/specs/DATA_MODEL.md` rather than ported from the old migrations, applied to a **new** Supabase project. Four domains, four migrations, four issues.

**Why it splits.** DATA-02 seeds the exercise library and touches the catalog only; as one node it waited on execution tables it never reads. The migrations apply in order — that is a property of foreign keys, not of the tickets — but a dependent should wait for the domain it uses, not for all four.

The structural changes that make D6 impossible and the three-state model real are distributed across the domains below: **blocks** as a first-class level (c) · **discriminated prescriptions** replacing a TEXT `reps` column that held four data types (c) · **temporal lineage** with revision and execution status kept separate (c) · **typed absence** (d).
...
### DATA-01a — Schema: catalog domain
...
### DATA-01b — Schema: user baseline domain
**Layer:** data · **Milestone:** M0 · **Carry-over:** rebuild
**Depends on:** DATA-01a
**Spec:** `docs/specs/DATA_MODEL.md` §2
...
### DATA-01c — Schema: workout domain
**Layer:** data · **Milestone:** M0 · **Carry-over:** rebuild
**Depends on:** DATA-01b
...
### DATA-01d — Schema: execution domain
**Layer:** data · **Milestone:** M0 · **Carry-over:** rebuild
**Depends on:** DATA-01c
...
**Spec:** `docs/specs/generation/GENERATION_CONTRACT.md` §5
...
**Layer:** state · **Milestone:** M0 · **Carry-over:** new
**Depends on:** CORE-01
**Spec:** `docs/specs/IA.md` §5 cross-cutting — the four-state contract is realized per screen
...
### CORE-05 — Accessibility contract
**Layer:** state · **Milestone:** M0 · **Carry-over:** new
**Depends on:** ENV-01, DS-01
**Spec:** `docs/specs/design/ATOMIC.md` §9 · export `docs/design/exports/clear-design-system-0.5.0/docs/patterns.md`
...
**Layer:** state · **Milestone:** M0 · **Carry-over:** rebuild
**Depends on:** ENV-01, DATA-03
**Spec:** `docs/specs/DATA_MODEL.md` §5 profiles
...
> doesn't cover." `docs/specs/design/ATOMIC.md` is the substrate contract; read it before any
> DS or UI ticket.
>
...
**Depends on:** ENV-01
**Spec:** `docs/specs/design/ATOMIC.md` · `clear-design-system@0.5.0` (external input)
...
- [ ] A test asserts the exported `VERSION` equals the version recorded in `docs/specs/design/ATOMIC.md` §1
...
### DS-04 — App-composed controls *(split: DS-04a…c)*
**Spec:** `docs/specs/design/ATOMIC.md` §11

The three parts CLEAR needs that `clear-design-system@0.5.0` does not ship. Each is a **composition** of shipped tokens and classes, not a new visual idea.

**Why it splits.** Their dependents are disjoint. Card is on nearly every screen; Select is on History alone; CollapsibleSection is on Review and Workout. Bundled, HOME-01 waits on a filter control it never renders.
...
**Depends on:** DS-01
**Spec:** `docs/specs/design/ATOMIC.md` §5, §10 · export `docs/design/exports/clear-design-system-0.5.0/docs/patterns.md` pattern 3
...
**Depends on:** DS-01
**Spec:** `docs/specs/design/ATOMIC.md` §7.2 · `docs/specs/IA.md` §4
...
**Acceptance:**
- [ ] Every screen in `docs/specs/IA.md` §4 sets `data-atmosphere` to its documented level; a test asserts each route renders with that value
...
**Spec:** `docs/specs/design/ATOMIC.md`
...
**Layer:** design · **Milestone:** M0 · **Carry-over:** new
**Depends on:** DS-01
**Spec:** `docs/specs/design/ATOMIC.md` §13 · export `_adherence.oxlintrc.json`
...
**Spec:** `docs/specs/generation/GENERATION_CONTRACT.md` §9 errors
...
### GEN-02 — Workout generation: contract v4.1 *(split: GEN-02a…c)*
...
**Why it splits.** GEN-06 checks duration plausibility — a validation rule. As one node it waited on prompt authoring and a live Claude call it has nothing to do with. Retrieval is also pure SQL and testable against the seeded library with no model in the loop at all, which makes it the piece most worth landing first.

### GEN-02a — Candidate resolution + retrieval
**Layer:** api · **Milestone:** M1 · **Carry-over:** rebuild
...
**Spec:** `docs/specs/generation/GENERATION_CONTRACT.md` §4–5 · `docs/specs/generation/PROMPT_v4.md`
...
**Spec:** `docs/specs/generation/GENERATION_CONTRACT.md` §6–8 · `docs/specs/generation/WORKED_EXAMPLE.md`
...
**Spec:** `docs/specs/generation/GENERATION_CONTRACT.md` §9 · `docs/specs/IA.md` — Generate and Loading screen contracts
...
**Spec:** `docs/specs/design/ATOMIC.md` §5, §10 · export `docs/design/exports/clear-design-system-0.5.0/docs/patterns.md` pattern 2 · `docs/specs/screens/loading-screens.md`
...
> `docs/specs/screens/loading-screen-prototype.html` is now **reference for the copy sequence
...
### SES-01 — Session lifecycle + three-state persistence *(split: SES-01a…c)*

The workout state machine and the requirement that closes D6.

**Why it splits.** Streak derivation was folded in here because it is a query over completed sessions — but HOME-01 and HOME-02 need *only* that query, and as one node they waited on the whole state machine, the resume path and the reconstruction queries. That is the clearest case of false serialization in the graph.

### SES-01a — Session lifecycle + atomic persistence
**Layer:** state · **Milestone:** M1 · **Carry-over:** rebuild
**Depends on:** DATA-01d, DATA-03, CORE-03, AUTH-03
**Spec:** `docs/specs/DATA_MODEL.md` §7 · `docs/specs/generation/WORKED_EXAMPLE.md`
...
### SES-01c — Streak derivation
**Layer:** state · **Milestone:** M1 · **Carry-over:** rebuild
**Depends on:** SES-01a
**Spec:** `docs/specs/DATA_MODEL.md` §7
...
**Spec:** `docs/specs/structures/master-structure-clarity.md`
...
### EXE-04 — AMRAP + For Time + ladder renderers *(split: EXE-04a…c)*

The restructure specs built in. Three renderers, three structure types, no shared state between them — `SectionRenderer` dispatches and each one owns its own screen.

**Why it splits.** This is the split the whole `SectionRenderer` architecture exists to enable. The old app handled six structure types in one 652-line component; keeping the three hardest in one ticket would rebuild that coupling in the graph instead of in the file. Each renderer is a separate agent, a separate PR, a separate review.
...
### EXE-04a — Ladder renderer + rung selection
**Layer:** ui · **Milestone:** M1 · **Carry-over:** rebuild
**Depends on:** EXE-01
...
### EXE-04b — For Time renderer
**Layer:** ui · **Milestone:** M1 · **Carry-over:** rebuild
**Depends on:** EXE-01
**Spec:** `docs/specs/structures/ladder-for-time.md`
...
### EXE-04c — AMRAP renderer
**Layer:** ui · **Milestone:** M1 · **Carry-over:** rebuild
**Depends on:** EXE-01
**Spec:** `docs/specs/structures/amrap-logging.md`, `docs/specs/structures/quickfix-amrap-round-label.md`
...
**Spec:** `docs/specs/IA.md` — Workout screen contract · `docs/specs/DATA_MODEL.md` §6 rest fields
...
**Spec:** `docs/specs/DATA_MODEL.md` §5 set logs
...
**Layer:** ui · **Milestone:** M1 · **Carry-over:** rebuild
**Depends on:** SES-01a, DS-04a, DS-05
**Spec:** `docs/specs/IA.md` — Summary screen contract
...
**Spec:** `docs/specs/IA.md` — History and Session Detail screen contracts
...
**Spec:** `docs/specs/IA.md` — Home screen contract; all four states load-bearing
...
**Spec:** `docs/specs/favorites-v2.md` · `docs/specs/DATA_MODEL.md` §11 snapshot versioning · `docs/specs/IA.md`
...
**Spec:** `docs/specs/favorites-v2.md`
...
**Spec:** `docs/specs/DATA_MODEL.md` §5 — streak derived, never stored
...
**Spec:** `docs/specs/IA.md` — Settings screen contract
...
**Layer:** ui · **Milestone:** M2 · **Carry-over:** rebuild
**Depends on:** SET-01
**Spec:** `docs/specs/IA.md` — Settings sub-views
...
**Promoted — issue-ready.** The progressive-overload stub got its spec session (`docs/specs/OVR-01_progressive-overload.md`, 2026-08-24) and is now four session-sized requirements, per that spec's own slice order. Unit ambiguity — the spec's "blocks everything" flag — is resolved by DATA-01: `weight_unit` per set log plus a profile default, in the baseline schema.

### OVR-01 — Load anchors + progression rules *(split: OVR-01a…c)*

The spec's ~80%-of-value slice: e1RM load anchors from set logs, RPE-driven next-prescription rules, sparse/stale handling, and the weight suggestion on Review.

**Why it splits.** OVR-02 needs the rules; it does not need the Review surface. And the rule table is a set of pure functions with a test per row — the single most testable thing in the project, and the piece least helped by being bundled with UI.

### OVR-01a — Load anchors
**Layer:** state · **Milestone:** M3 · **Carry-over:** new
**Depends on:** EXE-02, SES-01b
**Spec:** `docs/specs/OVR-01_progressive-overload.md` §1
...
### OVR-01b — Progression rules
**Layer:** state · **Milestone:** M3 · **Carry-over:** new
**Depends on:** OVR-01a
**Spec:** `docs/specs/OVR-01_progressive-overload.md` §2, §5
...
### OVR-01c — Review surface: suggestion + "why this number"
**Layer:** ui · **Milestone:** M3 · **Carry-over:** new
**Depends on:** OVR-01b, REV-01, DS-05
**Spec:** `docs/specs/OVR-01_progressive-overload.md` §3
...
**Spec:** `docs/specs/OVR-01_progressive-overload.md` (§4, slice d)
...
**Spec:** `docs/specs/DATA_MODEL.md` §4 lineage · `docs/specs/generation/exercise-swap.md`
...
folded.** The reasoning for each cut is recorded in `docs/requirements/DEFERRED.md` so the same
...
**Accessibility.** The component layer is guaranteed by the design system (`docs/specs/design/ATOMIC.md` §9) and the cross-screen mechanisms by CORE-05. What remains is a review criterion on every UI issue, and it is not negotiable by deadline:
...
**The issue scripts are generated, not written.** `scripts/gen-issues.py` parses this file and emits `docs/github/02-create-issues.sh`, `docs/github/03-wire-dependencies.sh`, and every `docs/github/bodies/*.md`. It also validates the graph — unknown dependencies, cycles, the ready queue, widest fan-in and fan-out — and names body files whose requirement no longer exists. Run it after any edit here; `--check` validates without writing and exits non-zero on failure, which is what CI runs.
...
**The DAG is generated, never drawn.** `docs/DAG.md` (Mermaid) is produced from `gh issue list --json` + dependency data. Regenerate any time; hand edits are meaningless because the next generation overwrites them.
