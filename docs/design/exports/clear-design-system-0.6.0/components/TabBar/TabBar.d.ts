 ...
 *
 * Composes `OverflowRail`, so a tab set that cannot fit its width stays
 * reachable by horizontal scrolling with edge cues, and the active tab is
 * revealed automatically. Nothing to enable, and the tabs contract is unchanged.
 * TabBar is always a tablist — it never switches to navigation semantics. For
 * route navigation, compose `OverflowRail` inside your own `<nav>`.
 */
