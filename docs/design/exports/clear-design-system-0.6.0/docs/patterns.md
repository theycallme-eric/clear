...
**Don't** Use an assertive announcement for a success. Show a failure with no action. Say "something went wrong". Put a red border on a structure-tinted panel — a failure frame is an urgency frame on both layers (see *Frame role coherence* below).
...
## Frame role coherence

Applies to every pattern above, and to anything new.

A frame paints two layers — a border and a surface — and they must speak for the same role. **Border at full strength, surface at the 5–15% rung of the same role, composited over `--base`.**

The failure case is specific. Border colour is the layer that gets swapped per state: `--border-toast-negative` on a failure, `--border-selected` on a selection, a passed-in `borderColor` on a `ChamferedFrame`. The surface is the layer people forget, so it stays on `--surface-card` — which reads from **structure**. The result is an orange-tinted panel inside a red, green or blue frame: two layers disagreeing about which role is speaking, and a user who cannot tell which one to believe.

Pick the role, not two colours:

| Building with | Write |
| --- | --- |
| `ChamferedFrame` | `role="urgency"` — resolves both layers |
| The chamfer class | `class="clr-chamfer clr-chamfer--urgency"` |
| Raw CSS | `--surface-frame-urgency` + `--border-frame-urgency` |

Roles are `structure` · `interaction` · `selection` · `urgency` · `info`. Rungs are `-quiet` (5%), default (10%) and `-strong` (15%). `surfaceColor` / `borderColor` remain as escape hatches; passing `borderColor` alone derives the surface from it rather than falling back to structure.

Component tokens read from the pairs, so the role a component claims is visible in its token definition. Where a component sits deliberately off the rung — the low-time timer at 20%, the positive toast at 20% — the exception is annotated in `foundation.css`. An unannotated bare rung on a frame token is drift.

**This does not loosen the two rules above it.** Still 2–3 colours in view at once: a role frame is one of them, and five roles on one screen is four too many. Still never colour alone: toast severity carries a glyph, a selected chip carries a tick, an invalid field carries error text. The frame role is a reinforcing cue, never the only one.
