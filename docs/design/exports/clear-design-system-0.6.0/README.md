...
- **Origin repo:** `theycallme-eric/clear-app` (branch `main`), imported into `_source/` as read-only reference for the workout application's original implementation.
...
| `_source/` | Read-only reference pasted from the origin repo. Never edit, never re-export. |
| `ui_kits/app/` | A worked example app — the representative workout consumer, not a library surface. |
...
CSS-only parts in the foundation: `.clr-chamfer`, `.clr-card` (accent bar + body), `.clr-btn` (secondary / primary / pressed / disabled), `.clr-atmosphere`, `.clr-rail`.
...
Shipped as classes, all using spacing tokens: `.clr-shell` + `.clr-shell__content` (a centred content column), `.clr-stack` / `.clr-stack--tight` / `.clr-row` (gap-spaced groups), `.clr-atmosphere--fixed` (the background layer pinned behind an app), `.clr-rail` (the overflow rail). The atmosphere and grain/scanline overlays sit behind `z-index: 1` content.
...
Neither behaviour is a visual change at comfortable widths. Both are visible in the **Responsive Constraints** card, which measures overflow rather than asserting it.
...
The workout application composes cards as **LeftColumn + ChamferedFrame**:
...
- Inner padding uses the spacing scale.

Surfaces: low-alpha tint of the structure role. Border: solid structure. Accent bar: higher-alpha tint of the same role. Not a system component — `ChamferedFrame` is, and this is one composition of it.
