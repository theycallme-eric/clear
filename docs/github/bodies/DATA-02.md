...
Transform the captured catalog in the reused project into the new CLEAR schema. **Plus the taxonomy equivalence check** — the anchor split is a separate staged migration, and this is where it is proven safe before the old structures are dropped.

A dev-only flag seeds Eric's profile + default location so the M1 loop is usable before onboarding exists (ONB-01 is M2).

## Acceptance
- [ ] Row counts match the approved live snapshot — **140 exercises**, 150 anchor links, 488 muscle mappings, and 27 legacy movement-pattern rows
- [ ] The reviewed `component_movements` and `exercise_role` tags from migration evidence are reconciled onto all 140 exercises; they are not misrepresented as live columns
