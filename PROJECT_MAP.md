...
| `scripts/e2e/` | ENV-07's privileged half: the GoTrue-admin/PostgREST client, the deterministic test namespace, and the seed/reset lifecycle (`npm run e2e:seed` / `e2e:reset`). The only code in the repository that touches the service-role key, and it runs in Node, never in a browser | changing who the test users are, what they own, or how a session is minted; never for assertions, which are `e2e/`'s |
...
the live OTP/RLS lifecycle runs only from reviewed `main`, resetting its deterministic two-user
namespace before and after. The two halves meet only inside a spec. Nothing
under `src/` imports either one, and `src/test/e2e-harness.test.ts` fails if something starts to —
which is how "no test-only branches in `src/`" stays structural rather than remembered.
