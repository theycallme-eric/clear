...
- **Unit Swap** — Replace grouped exercises as a single unit (supersets, EMOM, AMRAP, For Time)
...
| For Time / Ladder | Swap the entire block as a unit | One swap icon on the group |
...
### Endpoint: `POST /generate/section` (Extended)

Extends the existing endpoint with swap-specific fields. The AI receives full context about what's staying so the replacement fits.
...
// Request
{
  ...
  swap_target: {
    ...
    structure_type?: string;     // 'superset' | 'emom' | 'amrap' | 'for_time' | 'ladder'
  };
...
**Unit swap (superset, EMOM, AMRAP, For Time):**
```
SWAP MODE: unit
...
**For unit groups (superset, EMOM, AMRAP, For Time):**
```
┌─────────────────────────────────────┐
│ SUPERSET                        ▲   │
│                                     │
...
- Label text adapts: "Swap" for single, "Swap Pair" for superset, "Swap Block" for EMOM/AMRAP/For Time
...
- [ ] EMOM/AMRAP/For Time blocks show one swap icon for the group
